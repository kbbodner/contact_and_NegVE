...
#          SEIR Sensitivity Analyses - Data for Contour Plots           #
#                                                                       #
########################################################################
...
##DATA GENERATION FOR FIGURE 2 (a and b)
#SENSITIVITY ANALYSES USING AN SEIR MODEL AND 300 TIMESTEPS
#RECORDS MINIMUM VACCINE EFFECTIVENESS MEASUREMENT TO PLOT THE DEGREE OF
#NEGATIVE VE (CALCULTED BASED ON THE RELATIVE RISK)

#veT.contour conducts sensitivity analyses using:
#vaccine efficacy against susceptibility (VES) and
#vaccine efficacy against infectiousness (VEI)
#INPUTS
#vaccine.type="AllOrNothing" - assumes a take all vaccine type as default
#Assumes vaccinated contact heterogeneity (50% higher contact among vaccinated)

veT.contour<-function(vaccine.type="AllOrNothing"){
...
#Generates scenarios (can take a while to run) for FIGURE 2a
VeT.contour.df<-veT.contour(vaccine.type="AllOrNothing")

#write.csv(VeT.contour.df,"VES_and_VEI_SEIR.csv")

#veS.contact.contour conducts sensitivity analyses using:
#vaccine efficacy against susceptibility (VES) and
#vaccinated contact heterogeneity (from 0% difference [random mixing] to 100% higher contact)
#INPUTS
#vaccine.type="AllOrNothing" - assumes a take all vaccine type as default
...
#Generates scenarios(can take a while to run) for FIGURE 2b
VeS_Contact.contour.df<-veS.contact.contour(vaccine.type="AllOrNothing")

#write.csv(VeS_Contact.contour.df,"VES_and_ContactIncrease_SEIR.csv")

###DATA GENERATION FOR SUPPLEMENTARY WEB FIGURE 5 (UNDERESTIMATION CONTOURS)
#CONDUCTS SENSITIVITY ANALYSES for MAXIMUM UNDERESTIMATE OF VE (BASED ON THE RELATIVE RISK)

#VeI.contact.contour.underestimtation (based on an SEIR model and 300 timesteps)
#records the minimum vaccine effectiveness measured under different levels of 
#vaccine efficacy against infectiousness (VEI) and vaccinated contact heterogeneity
#INPUTS
#vaccine.type="AllOrNothing" - assumes a take all vaccine type as default
#ve.susc - level of vaccine efficacy against susceptibility (i.e. VES)


veI.contact.contour.underestimation<-function(vaccine.type="AllOrNothing",ve.susc=0.1){
  ...
  Ntot   <- npop*f            # population size in each subgroup (vector of 2 elements)
  ngroup <- length(f)         # length of vector representing number of subgroups
  I_init    <- c(3,1)         # 3,1 infectious individual per group
  E_init    <- c(0,0)         # 0 exposed individuals
...
#Contour Plot for VES=0.1
veI.contact.under.ve.susc0.1<-veI.contact.contour.underestimation(vaccine.type="AllOrNothing",ve.susc=0.1)
#write.csv(veI.contact.under.ve.susc0.1,"VEI_and_ContactIncrease_Under_VES0_1.csv")

#Contour Plot for VES=0.3
veI.contact.under.ve.susc0.3<-veI.contact.contour.underestimation(vaccine.type="AllOrNothing",ve.susc=0.3)
#write.csv(veI.contact.under.ve.susc0.3,"VEI_and_ContactIncrease_Under_VES0_3.csv")

#Contour Plot for VES=0.5
veI.contact.under.ve.susc0.5<-veI.contact.contour.underestimation(vaccine.type="AllOrNothing",ve.susc=0.5)
#write.csv(veI.contact.under.ve.susc0.5,"VEI_and_ContactIncrease_Under_VES0_5.csv")

#Contour Plot for VES=0.7
veI.contact.under.ve.susc0.7<-veI.contact.contour.underestimation(vaccine.type="AllOrNothing",ve.susc=0.7)
#write.csv(veI.contact.under.ve.susc0.7,"VEI_and_ContactIncrease_Under_VES0_7.csv")

#Contour Plot for VES=0.9
veI.contact.under.ve.susc0.9<-veI.contact.contour.underestimation(vaccine.type="AllOrNothing",ve.susc=0.9)
#write.csv(veI.contact.under.ve.susc0.9,"VEI_and_ContactIncrease_Under_VES0_9.csv")
